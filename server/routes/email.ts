...
import { SendEmailRequest, SendEmailResponse, EmailHistoryItem } from "@shared/api";
...
export const handleSendEmail: RequestHandler = async (req, res) => {
  try {
    const { to, subject, message, isHtml, template }: SendEmailRequest = req.body;
    ...
    if (lastEmail && lastEmail.status === 'pending') {
      ...
      lastEmail.error = error instanceof Error ? error.message : 'Unknown error';
    }
